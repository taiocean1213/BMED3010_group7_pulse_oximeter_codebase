...
Filter<element_data_type, frequency_datatype>::Filter(
    ...
    : fft_(fft) {
  // Initialize the filter with the provided passbands, stopbands, sampling
  // frequency, and FFT instance. The actual implementation depends on the
...
Filter<element_data_type, frequency_datatype>::filter(
  ...
  std::vector<element_data_type> transformedInput = fft_.transform(filterInput);
  ...
  for (const auto& data : transformedInput) {
    if (true/* FIXME data is within the passbands and not within the stopbands */) {
